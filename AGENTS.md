...
**V3.0**：View 收敛为 `aios-home` / `aios-workspace` / `web-operator` / `office` / `external-browser:*`；**V1.3** 增加 `task-workbench`（本地任务三栏，直调 copilot-serve）。Hermes 运维迁入 **TopBar Settings Drawer**（`modules/hermes-runtime/`）；初版 `LoginGate` + mock Auth/Bootstrap（**V3.3 起改为 splash→login 前置 + 真实 AI-OS HTTP Auth**）；`aios-home` → `http://127.0.0.1:3000`（无 `/zh`）。
