...
| `window.mainPageState` | `src/preload/main-page-state-api.ts` | MainPage 壳状态读写（tabOrder / externalTabs / sidebarMode） |
...
| **AI-OS Workspace** | 侧栏 / Tab | `screens/AIOSWorkspace/AIOSWorkspaceScreen.tsx` |
| Web Operator | 侧栏 / Tab | `screens/WebOperator/WebOperatorScreen.tsx` |
| Office | 侧栏 | `screens/Office/Office.tsx` |
| Hermes Runtime | TopBar Settings Drawer | `modules/hermes-runtime/*`（非页面路由） |
...
| **V3.0** | View 收敛、Hermes Runtime Drawer、LoginGate、`desktopAuth` / `desktopUserConfig`（mock） | `modules/hermes-runtime/`, `modules/auth/`, `main/auth/`, `main/user-config/`, `auth-api.ts`, `user-config-api.ts` |
