...
export function RuntimePanel(): React.JSX.Element {
  const { t } = useI18n();
  const { activeProfileId, runtime } = useAIOSWorkspace();
  ...
  const loadLogs = useCallback(async () => {
    ...
    try {
      const entries = await aiosWorkspaceApi.getGatewayLogs(activeProfileId, { limit: 80 });
  ...
  const loadAudit = useCallback(async () => {
    ...
    try {
      const rows = await aiosWorkspaceApi.listAuditEvents(activeProfileId, 30);
      setAuditRows(
        rows.map((r) => ({
          id: r.id,
  ...
  useEffect(() => {
    ...
    const unsub = aiosWorkspaceApi.onRuntimeStatusChanged((ev) => {
  ...
  if (!activeProfileId) {
    return <p className="p-3 text-xs text-gray-500">{t("aiosWorkspace.noProfile")}</p>;
  }
  ...
  const healthLabel =
    runtime.status !== "running"
      ? "—"
      : runtime.healthy
        ? t("aiosWorkspace.runtime.healthOk", { defaultValue: "OK" })
        : t("aiosWorkspace.runtime.healthUnhealthy", { defaultValue: "Unhealthy" });
    ...
    <div className="flex h-full min-h-0 flex-col overflow-hidden p-3 text-xs text-gray-300">
      <dl className="shrink-0 space-y-1">
        <div className="flex justify-between">
          <dt className="text-gray-500">{t("aiosWorkspace.runtime.port", { defaultValue: "Port" })}</dt>
          <dd>{runtime.port ?? "—"}</dd>
        </div>
        <div className="flex justify-between">
        ...
        <div className="flex justify-between">
          <dt className="text-gray-500">{t("aiosWorkspace.runtime.health", { defaultValue: "Health" })}</dt>
      ...
      <div className="mt-3 flex shrink-0 flex-wrap gap-2">
        ...
        >
          {t("aiosWorkspace.runtime.start", { defaultValue: "Start" })}
        </button>
        <button
          type="button"
        ...
        >
          {t("aiosWorkspace.runtime.stop", { defaultValue: "Stop" })}
        </button>
        <button
          type="button"
          className="rounded bg-gray-700 px-2 py-1 hover:bg-gray-600 disabled:opacity-50"
      ...
      <section className="mt-3 min-h-0 flex-1 overflow-y-auto">
        <h4 className="mb-1 font-semibold text-gray-400">
          {t("aiosWorkspace.runtime.events", { defaultValue: "Events" })}
        </h4>
        {profileEvents.length === 0 ? (
          <p className="mb-3 text-[10px] text-gray-600">
            {t("aiosWorkspace.runtime.noEvents", { defaultValue: "No recent status changes" })}
          </p>
        ) : (
        ...
        <h4 className="mb-1 font-semibold text-gray-400">
          {t("aiosWorkspace.runtime.audit", { defaultValue: "Audit" })}
        </h4>
        {auditRows.length === 0 ? (
          <p className="mb-3 text-[10px] text-gray-600">
            {t("aiosWorkspace.runtime.noAudit", { defaultValue: "No audit events" })}
          </p>
        ) : (
          <ul className="mb-3 space-y-1 text-[10px]">
            {auditRows.map((row) => (
        ...
        <h4 className="mb-1 font-semibold text-gray-400">
          {t("aiosWorkspace.runtime.logs", { defaultValue: "Logs" })}
        </h4>
        <pre className="rounded bg-gray-950 p-2 text-[10px] text-gray-400 whitespace-pre-wrap">
          {logs || t("aiosWorkspace.runtime.noLogs", { defaultValue: "No logs" })}
        </pre>
      </section>
    </div>
  );
