...
export const aiosWorkspaceApi = {
  ...
  async listWorkspaceFiles(profileId: string, relativePath = "."): Promise<WorkspaceFileEntry[]> {
    return (await window.aiosWorkspace.listFiles(profileId, relativePath)) as WorkspaceFileEntry[];
  },
  ...
  > {
    return window.aiosWorkspace.readFile(profileId, relativePath);
  },

  async getWorkspaceGitStatus(profileId: string): Promise<WorkspaceGitStatus> {
    return window.aiosWorkspace.gitStatus(profileId);
  },
};
