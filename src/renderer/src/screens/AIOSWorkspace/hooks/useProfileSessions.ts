...
/** Single sessions fetch per AIOS Workspace shell — call only from AIOSWorkspaceProvider. */
export function useProfileSessions(
  ...
  const refetch = useCallback(async () => {
    ...
    setError(null);
    try {
      const rows = await aiosWorkspaceApi.listSessions(profileId);
  ...
  const renameSession = useCallback(async (sessionId: string, title: string) => {
    await aiosWorkspaceApi.renameSession(sessionId, title);
    setSessions((prev) =>
      prev.map((s) =>
  ...
  const deleteSession = useCallback(
    async (sessionId: string) => {
      if (!profileId) return;
      await aiosWorkspaceApi.deleteSession(profileId, sessionId);
