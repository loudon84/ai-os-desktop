...
export function useHermesChatStream(
  ...
  useEffect(() => {
    const unsubChunk = aiosWorkspaceApi.onMessageChunk((chunk) => {
      if (!acceptsStreamEvents()) return;
      setRunState("streaming");
      setStreamingContent((prev) => prev + chunk);
    });
    const unsubDone = aiosWorkspaceApi.onMessageComplete((id) => {
      if (!acceptsStreamEvents()) return;
    ...
    const unsubError = aiosWorkspaceApi.onMessageError((err) => {
      if (!acceptsStreamEvents()) return;
      setMessages((msgs) => [
        ...msgs,
        {
    ...
    const unsubTool = aiosWorkspaceApi.onToolProgress((tool) => {
      if (!acceptsStreamEvents()) return;
  ...
  const loadSessionHistory = useCallback(async (sid: string) => {
    try {
      const rows = await aiosWorkspaceApi.getSessionMessages(sid);
      setMessages(
        rows.map((m) => ({
          id: String(m.id),
          sessionId: sid,
  ...
  const send = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        await aiosWorkspaceApi.sendMessage(
          profileId,
          trimmed,
          sessionRef.current,
          history,
  ...
  const cancel = useCallback(async () => {
    await aiosWorkspaceApi.abortChat();
    streamingOwnerRef.current = null;
    setStreamingContent("");
