...
/** Single runtime poll per AIOS Workspace shell — call only from AIOSWorkspaceProvider. */
export function useProfileRuntime(
  ...
  const refresh = useCallback(async () => {
    if (!profileId) return;
    try {
      const states = await aiosWorkspaceApi.getRuntimeStatus();
      ...
      let healthy = false;
      if (status === "running") {
        const probe = await aiosWorkspaceApi.probeProfileHealth(profileId);
  ...
  return {
    ...
    lastError,
    actionLoading,
    start: () => runAction(() => aiosWorkspaceApi.startProfile(profileId!)),
    stop: () => runAction(() => aiosWorkspaceApi.stopProfile(profileId!)),
    restart: () => runAction(() => aiosWorkspaceApi.restartProfile(profileId!)),
    refresh,
  };
}
