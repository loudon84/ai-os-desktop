...
export function useWorkspaceTree(profileId: string | null): {
  ...
  const refetch = useCallback(async () => {
    ...
    try {
      const [list, home, git] = await Promise.all([
        aiosWorkspaceApi.listWorkspaceFiles(profileId, currentPath),
        aiosWorkspaceApi.getHermesHome(profileId),
        aiosWorkspaceApi.getWorkspaceGitStatus(profileId),
      ]);
      setEntries(list);
      setHermesHome(home);
      setGitStatus(git);
  ...
  const openFile = useCallback(
    async (path: string) => {
      if (!profileId) return;
      const result = await aiosWorkspaceApi.readWorkspaceFile(profileId, path);
      if (result.ok) {
        setPreview({
          path,
