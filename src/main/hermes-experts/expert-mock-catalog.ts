...
export const MOCK_EXPERT_CATALOG: HermesExpert[] = [
  expert({
...
export function getMockExpert(expertId: string): HermesExpert | null {
  return MOCK_EXPERT_CATALOG.find((e) => e.expertId === expertId) ?? null;
}

export function getMockTeam(teamId: string): HermesExpertTeam | null {
  return MOCK_EXPERT_TEAMS.find((t) => t.teamId === teamId) ?? null;
}
