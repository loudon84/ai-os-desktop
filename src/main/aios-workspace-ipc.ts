...
export function setupAiosWorkspaceIPC(): void {
  ipcMain.handle("aios-workspace:git-status", async (_event, profileId: string) => {
    const home = profileHomeDir(profileId);
    return queryGitStatus(home);
  });

  ipcMain.handle(
    "aios-workspace:list-files",
