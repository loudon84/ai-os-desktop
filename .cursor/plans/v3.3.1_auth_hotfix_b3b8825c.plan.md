...
> 所有连接模式均要求 Desktop Auth（endpoint + token）与 bootstrap 完成后，才进入 main；AI-OS Home token 注入仅作用于 `persist:aios-home` 分区。

### 8. CHANGELOG

仓库无现有 CHANGELOG — **新增** [`CHANGELOG.md`](CHANGELOG.md)（根目录），首条 `[0.3.6] - V3.3.1`：

- `persist:aios-desktop` → `persist:aios-home`（用户需重新登录 AI-OS Portal）
