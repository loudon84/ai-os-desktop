---
name: V3 模块重构计划
overview: 按 [prd/v3.1_module.md](prd/v3.1_module.md) 将 Hermes Desktop 收敛为「AI-OS 主应用 + 4 个 Shell View + Hermes Runtime Drawer + 登录/配置 Bootstrap」，分 6 个阶段交付；Auth/Bootstrap 先实现 Main/Preload 契约与 mock，待 AI-OS backend 就绪后切换真实 HTTP 客户端。
todos:
  - id: phase-1-view-url
...
1. 缓存 remote config → 2. Hermes connection/profiles/models/soul/memory/toolsets → 3. AI-OS env + `writeAiOsEnvFile` → 4. `reconcileAiOsRuntime` / optional `startAiOs` → 5. `restartGateway` if needed → 6. 写 `bootstrap-state.json`
