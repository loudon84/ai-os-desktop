...
路径解析入口：`src/main/runtime/runtime-paths.ts`（`resolveCopilotRuntimePaths`）。旧布局（`hermes-agent` / `copilot-serve` / `ai-os-full`）在路径解析时作**读取 fallback**；App 启动 schema v4 迁移（`004-v53-runtime-layout.ts`）负责一次性磁盘搬迁至 canonical 布局。
