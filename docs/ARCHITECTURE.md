...
**运维部署（V5.3.4）：** `build/scripts/deploy-copilot-serve.ps1` 在 copilot-serve 流程之后 clone `https://github.com/loudon84/ai-os-full.git` 到 `runtime/portal/src`，执行 `pnpm install`，并 merge `desktop-runtime.json`。参数 `-SkipPortal` / `-SkipServe` 可拆分部署。
